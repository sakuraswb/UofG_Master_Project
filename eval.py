...
def evaluate_iou(test_preds, test_target_img_paths, num_classes=3, img_size=(160, 160), verbose_index=None):
    all_ious = []

    for i in range(len(test_preds)):
        pred_mask = np.argmax(test_preds[i], axis=-1).astype(np.uint8)
        gt_mask = load_img(test_target_img_paths[i], target_size=img_size, color_mode="grayscale")
        gt_mask = np.array(gt_mask).astype(np.uint8) - 1
...
def evaluate_ssim_scores(test_preds, test_target_img_paths, img_size=(160, 160), verbose_index=None):
